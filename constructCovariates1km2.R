...
##################
## Save covariates
save(distHgwCov, roadLengthCov, file = "outputs/covariatesRoads1km.RData")
...
# # "Communes" polygons
# communes <- st_read("data/tally/communes.geojson")
# 
# ####################
# ## Prey availability
# # Merge deer number with "communes"
# communes$nom <- str_to_lower(communes$nom) # to lowercase
# communes$nom <- stringi::stri_trans_general(communes$nom, "Latin-ASCII") # remove accent
# 
...
# 
# # cutSeq2 <- seq(1, 184433, 500) 
# # cutCells2 <- list()
# # for(i in 1:(length(cutSeq2) - 1)){
# #   cutCells2[[i]] <- seq(cutSeq2[i], cutSeq2[i + 1] - 1, by = 1)
# # }
# # cutCells2[[369]] <- 184001:184433
# 
...
# }
# 
# # Keep the unique polygons
# communes_unique <- distinct(communes_all[,"code_insee"])
# # gridPreys <- st_intersection(gridFr_sf[cutCells2[[1]],], communes_unique)
# # for(j in 2:length(cutCells2)){
# #   gridPreysCut <- st_intersection(gridFr_sf[cutCells2[[j]],], communes_unique)
# #   gridPreys <- rbind(gridPreys, gridPreysCut)
# #   print(j)
# #   save(gridPreys, file = "outputs/gridPreys1km.RData")
# # } 
# 
# ## INTERSECTION DONE WITH QGIS - MUUUUCH FASTER
# #gridPreys <- st_intersection_faster(gridFr_sf, communes_unique)
# #save(gridPreys, file = "outputs/gridPreysTemp1km.RData")
# gridPreys <- st_read("outputs/gridPreysTemp1km.shp")
# gridPreys$area <- st_area(gridPreys)
# gridPreys <- merge(gridPreys, st_drop_geometry(communes_all), by = "code_insee")
# 
...
#   dplyr::select(IDbuffer)
# 
# save(gridPreys4, buffer, file = "outputs/preyTemp1km.RData")
# # Save computation time
# gridPreys4_ <- gridPreys4[gridPreys4$ID %in% gridPivot$ID,]
# gridPreys4_2 <- gridPreys4_[!st_is_empty(gridPreys4_),,drop=FALSE]
# 
# # bufferCells <- st_intersection(buffer[cutCells2[[1]],], gridPreys4_2)
# # for(j in 2:length(cutCells2)){
# #   bufferCellsCut <- st_intersection(buffer[cutCells2[[j]],], gridPreys4_2)
# #   bufferCells <- rbind(bufferCells, bufferCellsCut)
# #   print(j)
# # } 
# #bufferCells <- st_intersection_faster(buffer, gridPreys4_2)
# bufferCells <- st_read("outputs/bufferCells.shp")
# 
# preyCov <- gridPreys4 %>% 
#   st_drop_geometry() %>% 
#   as.data.frame() %>% 
...
#       dplyr::select(ID, as.character(y)) %>% 
#       filter(is.na(.[, 2])) %>% 
#       pull(ID)
#     
#     # Buffer of cells which are NA
#     bufferYearNA <- bufferCells %>% 
#       dplyr::select(IDbuffer, paste0("X", as.character(y))) %>%
#       filter(IDbuffer %in% cellYearNAID) %>% 
#       st_drop_geometry() %>% 
#       group_by(IDbuffer) %>% 
...
#     
#     print(sum(is.na(preyCov[, as.character(y)])))
#   }
#   print(y)
# }
# 
...
##################
## Save covariates
#save(hDensCov, triCov, preyCov, hfiCov, file = "outputs/covariatesOthers1km.RData")
save(hDensCov, triCov, hfiCov, file = "outputs/covariatesOthers1km.RData")
